#!/bin/bash
# Handler/launcher for one-shot stochastic pruning experiments (experiment 10 in main.py).
# Submits one SLURM array job per (model, dataset, sigma, pruning_rate) combination via
# slurm_SP_one_shot_pruning_run.sh. Modeled after slurm_SP_fine_tuning_handler.sh.
...
sbatch --nodes=1 --time=01:00:00 --array=1-5 --partition=gpu --mail-type=all --mail-user=sclaam@leeds.ac.uk \
  ...
  slurm_SP_one_shot_pruning_run.sh 10 128 "${sigma}" "${pruner}" "${model}" "${dataset}" "${pr}" "alternative" 0 "${name}" "${solution}"
}
