#!/bin/bash
# Worker script for SLURM (sbatch) jobs running experiment 10 from main.py:
...
#
# Positional arguments (mirrors slurm_SP_fine_tuning_run.sh so handlers stay consistent):
#   $1  experiment number (use 10 for one-shot stochastic pruning, 6 for deterministic)
#   $2  batch size
